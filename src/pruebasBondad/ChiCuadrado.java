    ...
    public void calcularEstadisticoPrueba(double[] fo, double[] fe, Intervalo intervalos) {
        ...
        for (int i = 0; i < fe.length; i++) {
            ...
            if (feAcumulada >= 5) {
                ...
                extremoIzquierdo = new Double(df.format(intervalosAcumulados.get(0)[0]));
                extremoDerecho = new Double(df.format(intervalosAcumulados.get(intervalosAcumulados.size() - 1)[1]));
                intervalosAgrupados.add("[" + extremoIzquierdo + ", " + extremoDerecho + "]");
